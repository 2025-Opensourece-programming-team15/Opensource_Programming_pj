...
# 우리가 만든 스크래퍼 모듈 임포트 (경로: src/scraper.py)
from src.scraper import get_integrated_search_data, get_regular_post_data
...
def query_controller(user_input):
    """
    사용자의 의도를 파악하여 JSON으로 반환하는 컨트롤러입니다.
    """
...
def query_analyst(user_input, data_summary):
    """
    ...
    아래는 DC 인사이드에서 수집된 관련 게시물 요약 데이터입니다:
    -----
    {data_summary}
    -----
    ...
    3. **부정 여론**: 유저들이 불만인 부분.
    4. **주요 논쟁**: 현재 갤러리에서 핫한 토픽이나 싸움 거리.
...
def fetch_data(keyword, gallery_name=None, gallery_id=None, start_page=1, end_page=2, sort='latest'):
    """
    갤러리 힌트 유무에 따라 최적의 스크래퍼를 선택하고, 실패 시 통합 검색으로 폴백합니다.
    """
    df = pd.DataFrame()
    status_msg = ""
...
st.title("🕵️‍♂️ DC Insight Chatbot")
st.caption("디시인사이드 실시간 여론 분석기 powered by Gemini")

# 세션 초기화: 대화 기록이 없으면 환영 메시지 추가
if "messages" not in st.session_state:
    st.session_state.messages = []
    welcome_msg = "안녕하세요! DC 여론 분석 봇입니다. 궁금한 게임, 인물, 이슈(예: '젠레스 존 제로 빛나 여론 어때?')를 물어봐주세요."
...
if prompt := st.chat_input("무엇을 분석해 드릴까요?"):
    ...
    with st.chat_message("assistant"):
        ...
        with st.status("🤔 의도를 파악하고 있습니다...", expanded=True) as status:
            ...
            intent = query_controller(prompt)
            ...
            if mode == "search":
                ...
                try:
                    df, log_msg = fetch_data(keyword=keyword, gallery_name=gallery_name, gallery_id=gallery_id, sort=sort)
                    ...
                    if not df.empty:
                        ...
                        with st.expander("📊 참고한 게시물 원본 데이터 확인"):
                            st.dataframe(
                                df[['Title', 'Content', 'GalleryID', 'PostURL']].head(20),
